...
class MedioPrazo(Escalonador):  # Medio Prazo
    ...
    def swap_out(self, processo, ram, secundaria):  # Chama remover processo
        if (processo.pcb.estado == "Pronto"):
            ...
            print(f'Processo de id: {
                  processo.pcb.id} movido Pronto para Pronto-Suspenso\n')
    ...
    def swap_in(self, processo, ram, secundaria) -> bool:  # Memória Secundária --> Memoria_RAM
        ...
        if (processo.pcb.estado == "Pronto-Suspenso"):
            ...
            print(f'Processo de id: {
                  processo.pcb.id} movido de Pronto-Suspenso para {processo.pcb.estado}\n')
            return flag
...
class MemoriaRam:
    ...
    # 2048Mb cada partição(particionamento fixo) ou seja terá 16 elementos
    memoria = []
...
class CPU:
    ...
    def __init__(self, identificador):
        # Identificador do número da  (processo.pcb.prioridade)
        self.id = identificador
    ...
    def executar_processo(self):
        ...
        if (self.processo.etapa < self.processo.f1):  # Ainda está executando
...
class Computador:
    ...
    with open('input.txt', 'r') as arquivo:
        for linha in arquivo:
            ...
            # Está depois do print de troca mas da no mesmo
